...
async function checkExistingReviewByEmail(email) {
  const response = await fetch(`http://localhost:1337/api/appeals?filters[email][$eq]=${email}`);
  const reviews = await response.json();
  const lele = reviews.data
   return lele.length > 0; // returns true if any reviews exist with the given email
}
...
function Todo() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!articleName || !content || !selectValue) {
    ...
    const existingReview = await checkExistingReviewByEmail(user.primaryEmailAddress.emailAddress);
    if (existingReview) {
      alert("You have already submitted a review with this email.");
      return;
    }
    ...
    const Appeal = {
      data: {
        Name: articleName,
        FurtherInformation: content,
        Email: userEmail,
        Type: selectValue,
        PlacesWork: Description,
      },
    };

    try {
      const response = await fetch("http://localhost:1337/api/appeals", {
        method: "POST",
    ...
    <div className="flex justify-center">
      ...
      >
        <div className="flex flex-col ml-3 rounded-tl-md mb-2 rounded-tr-md align-top p-14 px-4 py-3 justify-start rounded-bt-md border-b border-r border-t border-l border-gray-700 text-black items-start w-full gap-3 font-medium text-[16px] pt-4 rounded-bl-md rounded-br-md outline-none">
          <h1 className=" ">Reviewer Appeal</h1>

          <div className=" ">
            <label className=" mr-2" htmlFor="articleName">
              Your Name:
            </label>
            <input
          ...
          <div className="flex align-top">
            <label className="mr-1 mb-2" htmlFor="Description">
              Where you Worked:
            </label>
            <textarea
              className="border border-t-gray-700 border-gray-700 rounded-md px-3 py-2 mt-1 focus:outline-none focus:ring focus:border-blue-300"
        ...
        <h1 className="mb-5">Further Information</h1>
