...
function ReaderTipTap({ article,User }) {
  ...
  const handleClick = async (event) => {
   ...
   try {
     ...
     alert("Article sent to revise  successfully!");
   } catch (error) {
     console.error("Error creating article:", error);
   }
   console.log(article)
  ...
  return (
    <div className="ReaderTipTapthings">
        ...
        <button onClick={handleReview} className="ml-5 group cursor-pointer outline-none" title="Add New">
          <svg
            className="mb-5  stroke-red-500 fill-none group-hover:fill-red-800 group-active:stroke-red-200 group-active:fill-teal-600 group-active:duration-0 duration-300"
            viewBox="0 0 24 24"
            height="50px"
            width="50px"
            xmlns="http://www.w3.org/2000/svg"
